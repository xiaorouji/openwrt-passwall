...
local function get_file_info(arch)
    ...
    if arch == "x86_64" then
        file_tree = "_linux_amd64"
    elseif arch == "aarch64" then
        file_tree = "_linux_arm64"
    elseif arch == "ramips" then
        file_tree = "_linux_mipsle"
    elseif arch == "ar71xx" then
        file_tree = "_linux_mips"
    elseif arch:match("^i[%d]86$") then
        file_tree = "_linux_386"
    elseif arch:match("^armv[5-8]") then
        file_tree = "_linux_arm"
...
function get_brook_file_path()
    return uci:get("passwall", "global_brook", "brook_client_file") or
               luci.sys.exec(
                   "echo -n `uci get passwall.@global_brook[0].brook_client_file`")
end
