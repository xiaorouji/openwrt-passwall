module("luci.model.cbi.v2ray_server.api.caddy", package.seeall)
...
local caddy_downloadurl = "https://caddyserver.com/download/linux/"
local caddy_downloadurl_params = "?license=personal&telemetry=on"
...
function get_caddy_file_path()
    return uci:get("v2ray_server", "global", "caddy_file") or
               luci.sys.exec(
                   "echo -n `uci get v2ray_server.@global[0].caddy_file`")
end

function get_caddy_version(file)
    if file == nil then file = get_caddy_file_path() end
    ...
    if file and file ~= "" then
        ...
        local info = util.trim(sys.exec("%s -version 2>/dev/null" % file))

        if info ~= "" then
            local tb = util.split(info, "%s+", nil, true)
            return tb[1] == "Caddy" and tb[2] or ""
        end
    end
...
function to_check(arch)
    ...
    local json = get_api_json(caddy_api)
    ...
    local client_file = get_caddy_file_path()

    local needs_update = compare_versions(get_caddy_version(client_file), "<",
                                          remote_version)
    local html_url, download_url

    if needs_update then
        html_url = json.html_url
        download_url = caddy_downloadurl .. file_tree .. sub_version ..
                           caddy_downloadurl_params
    end

    if needs_update and not download_url then
        return {
            code = 1,
            now_version = get_caddy_version(client_file),
...
function to_download(url)
    ...
    sys.call("/bin/rm -f /tmp/caddy_download.*")

    local tmp_file = util.trim(util.exec("mktemp -u -t caddy_download.XXXXXX"))
...
function to_extract(file, subfix)
    ...
    sys.call("/bin/rm -rf /tmp/caddy_extract.*")
    local tmp_dir = util.trim(util.exec("mktemp -d -t caddy_extract.XXXXXX"))
...
function to_move(file)
    ...
    local version = get_caddy_version(file)
    if version == "" then
        sys.call("/bin/rm -rf /tmp/caddy_extract.*")
        return {
            code = 1,
